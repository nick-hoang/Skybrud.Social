...
namespace Skybrud.Social.Json {
    ...
    public class JsonObject : IJsonObject {
        ...
        /// <summary>
        /// only accept object, not string, int or kind of
        /// </summary>
        /// <param name="jtoken"></param>
        public JsonObject(JToken jtoken)
        {            
            Dictionary = jtoken.ToObject<Dictionary<string, object>>();            
        }
        ...
        {                        
            if(Dictionary.TryGetValue(name, out var value) && value != null) {                
                return JsonConverter.ParseObject(value.ToString());                
            }
            return null;             
        }
        ...
        public JsonArray GetArray(string name)
        {
            if (Dictionary.TryGetValue(name, out var value) && value != null)
            {
                return JsonConverter.ParseArray(value.ToString());
            }
            return null;            
        }
        ...
        public T GetValue<T>(string name, IFormatProvider provider)
        {
            ...
            if (typeof(T) == typeof(JsonObject))
            {
                ...
                {
                    result = new JsonObject((Dictionary<string, object>)Dictionary[name]);
                }
                else result = JsonConverter.ParseObject(val.ToString());
                return (T)result;
            }
